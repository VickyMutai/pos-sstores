<?php
    ...
    if($id=$_GET['id']){
    $select = $pdo->prepare("SELECT * FROM product WHERE product_id=$id");
    $select->execute();
                ...
                if($img_ext == 'jpg' || $img_ext == 'jpeg' || $img_ext == 'png' || $img_ext == 'gif'){
                    if($img_size>= 1000000){
                        ...
                        if(move_uploaded_file($img_tmp,$store)){
                            $img_new;
                            if(!isset($error)){
                                $update = $pdo->prepare("UPDATE product SET product_code=:product_code,product_name=:product_name,
                                product_category=:product_category, sell_price=:sell_price,
                                stock=:stock,min_stock=:min_stock,client_name=:client_name ,description=:description, img=:img WHERE product_id=$id");
                                ...
                                $update->bindParam('client_name', $client_req);
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="category" required>
                                ...
                                extract($row);
                                ?>
                                    <option <?php if($row['category_name']==$category_db) {?>
                                    selected = "selected"
                                    <?php }?> >
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="client_name" required>
                                ...
                                extract($row);
                                ?>
                                    <option <?php if($row['client_name']==$client_db) {?>
                                    selected = "selected"
                                    <?php }?> >
